      ...
      for( var i = 0; i < ajUsers.length; i++){
        for( var j = 0 ; j < ajUsers[i].Tweets.length ; j++){
          ...
          <div class='twitt-profile-picture' ><img src="8.jpeg" alt="Profile Picture"></div>
            ...
            document
              .querySelector('.middle-btn')
              .insertAdjacentHTML('afterend', divTweet);
...
async function deleteTweet(){
  ...
  console.log(event)
  var tweetId = btn.getAttribute('data-form-id')
   console.log(tweetId)

  var dataDelete = new FormData(document.querySelector(`form[data-form-id="${tweetId}"]`));

    dataDelete.append('tweetId', tweetId );

    for(var pair of dataDelete.entries()){
  ...
  console.log(sResponse)
  //let ajResponse = JSON.stringify(sResponse);
...
//const tweetIdToUpdate = btn.parentNode.parentNode.getAttribute('data-tweet-id');


function openUpdateModal(){
 btn = event.target
  //console.log(btn)
  tweetIdToUpdate = btn.parentNode.parentNode.getAttribute('data-tweet-id');
...
async function updateTweet(){
  ...
  console.log(tweetIdToUpdate)
  var updateTweetInputField = document.getElementById("updateTweetInput").value
  console.log(updateTweetInputField)
  // var btn = event.target
  var tweetIdSelector = tweetIdToUpdate;
  //document.querySelector(`div [data-tweet-id="${tweetId}]`);
  console.log(tweetIdSelector)
  var tweetId = tweetIdToUpdate;
  console.log(tweetId)
    var dataUpdate = new FormData(document.querySelector(`form[data-tweet-id="${tweetId}"]`))
    dataUpdate.append('tweetId', tweetId);
    dataUpdate.append('updateTweetInputField', updateTweetInputField)
    ...
    //new FormData(document.querySelector(`form[data-form-id="${tweetId}"]`));

    let sResponse = await bridgeUpdate.text();

    let jResponse = await JSON.stringify(sResponse);
    console.log(jResponse)
    
    //document.querySelector(`#twitt-text-container[data-tweet-id=${tweetIdToUpdate}`).innerHTML = sResponse;
    document.querySelector(`div[data-updateTweet="${tweetId}"]`).innerHTML = sResponse;
